...
public class UniqueByPropertyConstraintValidator implements ConstraintValidator<UniqueByProperty, Collection> {
    ...
    public boolean isValid(Collection collection, ConstraintValidatorContext context) {
        ...
        List<Object> keys =
                StringUtils.hasText(getterName) ? getKeysByGetterMethod(collection) : getKeysByProperty(collection);
    ...
    private List<Object> getKeysByProperty(Collection collection) {
        ...
        return (List<Object>) collection.stream()
                .map(o -> {
                    try {
                        return targetField.get(o);
                    } catch (IllegalAccessException e) {
                        throw exception(
                                "Could not access property \"{0}\" on type {1}. Make sure the property is public.",
                                propertyName, first.getClass().getName());
                    }
                })
        ...
        return (List<Object>) collection.stream()
                .map(o -> {
                    try {
                        ...
                        throw exception(
                                "Could not access getter method \"{0}\" on type {1}. Make sure the method is public.",
                                getterName, first.getClass().getName());
                    } catch (InvocationTargetException e) {
                        String exceptionMessage = MessageFormat.format(
                                "Getter method \"{0}\" on type {1} threw exception: {2}",
